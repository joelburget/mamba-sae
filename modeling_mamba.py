# From https://github.com/LegallyCoder/mamba-hf/blob/main/src/modeling_mamba.py
...
class MambaRMSNorm(nn.Module):
    ...
    def forward(self, x):
        output = x * torch.rsqrt(x.pow(2).mean(-1, keepdim=True) +
...
class MambaBlock(nn.Module):
    ...
    def __init__(self, config: MambaConfig):
        ...
        self.in_proj = nn.Linear(config.d_model,
                                 config.d_inner * 2,
                                 bias=config.bias)
        ...
        self.x_proj = nn.Linear(config.d_inner,
        ...
        A = repeat(torch.arange(1, config.d_state + 1),
                   'n -> d n',
                   d=config.d_inner)
        self.A_log = nn.Parameter(torch.log(A))
        self.D = nn.Parameter(torch.ones(config.d_inner))
        self.out_proj = nn.Linear(config.d_inner,
                                  config.d_model,
                                  bias=config.bias)
    ...
    def ssm(self, x):
        ...
        (delta, B,
         C) = x_dbl.split(split_size=[self.config.dt_rank, n, n],
    ...
    def selective_scan(self, u, delta, A, B, C, D):
        ...
        deltaB_u = einsum(delta, B, u,
                          'b l d_in, b l n, b l d_in -> b d_in l n')
